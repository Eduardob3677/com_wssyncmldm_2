...
public class PeriodicHeartbeatManager {
    ...
    private void startTimer(long j3) {
        long jCurrentTimeMillis = System.currentTimeMillis();
        Log.I("heartbeat time is " + j3 + " and current time is " + jCurrentTimeMillis);
        if (j3 < jCurrentTimeMillis) {
            Log.W("heartbeat time is passed. start heartbeat alarm after 1 min");
            j3 = TimeConstants.MINUTE_IN_MILLIS + jCurrentTimeMillis;
        }
        ...
        alarmManager.setRepeating(1, j3, 43200000L, RestClientServiceType.PERIODIC_HEARTBEAT.getPendingIntent(this.context));
        Log.D("Start heartbeat timer: " + new Formatted.DateTimeForLog(j3));
    }
    ...
    public void startIfPossible(long j3) {
        if (j3 > 0) {
            startTimer(j3);
        } else {
            stopTimer();
