    ...
    public void doHandle(int i5, Object obj, int i6) {
        if (i5 == 10) {
            if (obj instanceof IDMDDDataInfo) {
                ...
                this.executeHandler.executeIfPossible(IDMCallbackStatusInterface.IDM_DL_STATE_DOWNLOAD_FAILED_REPORTING, EnablerUtils.convertDlResultCodeToDmResultCode(6));
                new IDMFumoTaskDownloadFailed(this.taskId).execute();
                return;
            }
        }
        if (i5 == 20) {
            if (this.actionInfoDao.getDlReportCode() == 2) {
            ...
            } else if (this.actionInfoDao.getDlReportCode() == 8) {
                new IDMFumoTaskDownloadFailed(this.taskId).execute();
            }
            this.executeHandler.callbackIfPossible(i5, (String) Optional.ofNullable(obj).map(new d(4)).orElse(null));
            return;
        }
        if (i5 == 30) {
