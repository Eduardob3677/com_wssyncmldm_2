...
public class IDMDynamicReceivers {
    ...
    public static class BearerChangeReceiver extends ReceiverWithIntentFilter {
        @Override // com.idm.fotaagent.receiver.dynamic.ReceiverWithIntentFilter
        public IntentFilter getIntentFilter() {
            return new IntentFilter(IntentActions.IDM_INTENT_CSC_FOTA_UPDATE);
        }

        @Override // com.idm.fotaagent.utils.receiver.SafeBroadcastReceiver
        public void handle() {
            Log.I(IntentActions.IDM_INTENT_CSC_FOTA_UPDATE);
            WifiOnlyChecker.getInstance().loadWifiOnlyByBearer();
        }
    }
                ...
                if (cls.isAnnotationPresent(ReceiverToBeAutoRegistered.class)) {
                    if (ReceiverWithIntentFilter.class.isAssignableFrom(cls)) {
                        ...
                        if (cls.isAnnotationPresent(ReceiverToBeSpecifiedFlag.class)) {
                            property = (ReceiverToBeSpecifiedFlag.Property) Optional.ofNullable((ReceiverToBeSpecifiedFlag) cls.getAnnotation(ReceiverToBeSpecifiedFlag.class)).map(new d(5)).orElse(property);
                        }
