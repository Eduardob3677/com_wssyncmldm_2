...
public enum Operator {
    ...
    TMB { // from class: com.idm.fotaagent.enabler.feature.Operator.5
        ...
        public boolean isWifiOnlyForDl(String str) {
            boolean z4 = new FotaJobRepository(IDMApplication.idmGetContext()).isOptionalService() && ((double) new ActionInfoDao(str).getObjectSize()) / 1000000.0d > 100.0d;
            if (z4) {
                Log.I("TMB + Optional + Bigger than 100MB => Wifi Only");
            }
            return z4;
        }
    },
    TMK { // from class: com.idm.fotaagent.enabler.feature.Operator.6
        ...
        public boolean isWifiOnlyForDl(String str) {
            boolean z4 = new FotaJobRepository(IDMApplication.idmGetContext()).isOptionalService() && ((double) new ActionInfoDao(str).getObjectSize()) / 1000000.0d > 100.0d;
            if (z4) {
                Log.I("TMK + Optional + Bigger than 100MB => Wifi Only");
            }
            return z4;
        }
    },
    ...
    public static class MockOperator extends Feature<Operator> {
        public MockOperator(Context context) {
            super(context, Operator.values(), new a(7), new b(18), new A2.a(14));
        }
    }
    ...
    private static Operator getDeviceOperator() {
        ...
        return new MockOperator(IDMApplication.idmGetContext()).getValue().orElseGet(new C2.b(1));
    }
