...
public class IDMAdapterDownload extends IDMDlBaseDownload {
    ...
    private void updateDownloadInfo(IDMDlDownloadInfo iDMDlDownloadInfo, long j3) {
        long j5 = this.downloadProgressSize;
        if (j5 <= 0 || j3 <= 0) {
            return;
        }
        iDMDlDownloadInfo.idmSetDownloadSize(j5);
        int i5 = (int) ((this.downloadProgressSize * 100) / j3);
    ...
    @Override // com.idm.agent.dl.IDMDlBaseDownload
    public boolean idmUpdateDownloadInfo(IDMDlDownloadInfo iDMDlDownloadInfo, long j3, byte[] bArr) throws IDMExceptionFileEnospc, IOException, IDMExceptionFileNotFound {
        this.downloadProgressSize += bArr.length;
        writeFirmwareObject(bArr, j3);
        updateDownloadInfo(iDMDlDownloadInfo, j3);
        return needsToRefreshProgress(j3);
    }

    public boolean needsToRefreshProgress(long j3) {
        long j5 = this.downloadProgressSize;
        int i5 = ((int) j5) / 524288;
    ...
    public void setDownloadedSize(long j3) {
        this.downloadProgressSize += j3;
    }

    public void writeFirmwareObject(byte[] bArr, long j3) throws IDMExceptionFileEnospc, IOException, IDMExceptionFileNotFound {
        int length = bArr.length;
        if (length > 0) {
            this.flexibleBuffer.idmAddFlexibleBuffer(bArr, length);
        }
        if (this.flexibleBuffer.idmGetFlexibleBufferLength() > this.flexibleBuffer.idmGetAppendSavedBufferSize() || (this.downloadProgressSize >= j3 && this.flexibleBuffer.idmGetFlexibleBufferLength() > 0)) {
