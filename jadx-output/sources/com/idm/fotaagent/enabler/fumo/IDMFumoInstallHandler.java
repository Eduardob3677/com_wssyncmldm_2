...
public class IDMFumoInstallHandler {
    ...
    public void checkPolicyAndInstall() throws IOException {
        try {
            try {
                try {
                    try {
                        ...
                        Policies.ForInstall.getExceptional().ifThrown(installSuspended.act(), installSuspended.getExceptionClass()).ifThrown(installFailed.act(), installFailed.getExceptionClass()).orElse(new Callable() { // from class: com.idm.fotaagent.enabler.fumo.e
                            ...
                            public final Object call() {
                                return Boolean.valueOf(this.f6380a.install());
                            }
                        });
                        ...
                        if (!TextUtils.isEmpty(e5.getMessage()) && e5.getMessage().contains("No space left on device")) {
                ...
                } catch (InsufficientMemoryException | InvalidStateException e6) {
            ...
            } catch (IDMFumoExceptionImproperFumostatus e7) {
                Log.printStackTrace(e7);
                NotificationTypeManager.cancel(NotificationId.NOTIFICATION_ID_PRIMARY);
                reportUpdateFailAndShowDialog(e7.getMessage() == null ? IDMDlInterface.IDM_FOTA_GENERIC_UPDATE_FAILED : e7.getMessage());
                IDMUIManager.getInstance().finish(IDMUIEventInterface.UIDIALOG.IDM_DIALOG_INSTALL_PLEASE_WAIT);
                new SilentReboot().disable();
                installRequested.set(false);
                if (!needToReboot.get()) {
                ...
                NotificationTypeManager.cancel(NotificationId.NOTIFICATION_ID_PRIMARY);
                reportUpdateFailAndShowDialog(e8.getMessage() == null ? "400" : e8.getMessage());
                IDMUIManager.getInstance().finish(IDMUIEventInterface.UIDIALOG.IDM_DIALOG_INSTALL_PLEASE_WAIT);
                new SilentReboot().disable();
                installRequested.set(false);
                if (!needToReboot.get()) {
    ...
    public void execute() {
        ...
        if (!atomicBoolean.compareAndSet(false, true)) {
            ...
            new Thread(new Runnable() { // from class: com.idm.fotaagent.enabler.fumo.d
                @Override // java.lang.Runnable
                public final void run() throws IOException {
                    this.f6379c.checkPolicyAndInstall();
                }
            }).start();
        } else {
