...
public class DLNetworkChecker extends NetworkChecker {
    ...
    public boolean shouldShow(ConfirmType confirmType, String str) {
        ...
        if (confirmType == ConfirmType.DOWNLOAD_CONFIRM) {
            if (new FotaJobRepository(IDMApplication.idmGetContext()).isOptionalService() && !actionInfoDao.getIsContinuousUpdateSession()) {
                Log.I("3. Optional Service Type -> show " + confirmType);
                return true;
            }
            if (CarrierFeature.isShowDeltaSizeBeforeDownload()) {
                Log.I("4. NotifyDeltaSizeBeforeDownload -> show " + confirmType);
                return true;
            }
        }
        ...
        if (zIsWiFiNetworkConnected && (autoDownloadOption == AutoDownloadOptions.WIFI || autoDownloadOption == AutoDownloadOptions.WIFI_AND_CELLULAR)) {
            Log.I("5-1. WiFiOnly + Auto download menu : " + autoDownloadOption + " -> do not show " + confirmType);
            return false;
        }
        Log.I("5-2. WiFiOnly + Auto download menu : " + autoDownloadOption + " and wifiConnected : " + zIsWiFiNetworkConnected + " -> show " + confirmType);
        return true;
    }
