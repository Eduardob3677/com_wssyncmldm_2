...
public abstract class HiddenCommand {
    ...
    public <T extends Enum<T>> void showDialog(String str, Enum<T>[] enumArr, Enum<T> r5, Consumer<Enum<T>> consumer) {
        new AlertDialog.Builder(this.context).setTitle(str).setSingleChoiceItems((CharSequence[]) Arrays.stream(enumArr).map(new G2.b(25)).toArray(new com.idm.fotaagent.database.sqlite.database.mo.b(1)), r5.ordinal(), (DialogInterface.OnClickListener) null).setPositiveButton(R.string.STR_BTN_OK, new b(consumer, 1, enumArr)).show();
    }
