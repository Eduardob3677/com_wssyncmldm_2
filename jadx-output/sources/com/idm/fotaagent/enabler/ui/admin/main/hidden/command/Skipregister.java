...
public class Skipregister extends HiddenCommand {
    ...
    public /* synthetic */ void lambda$execute$0(DeviceRegistrationRepository deviceRegistrationRepository, RegisteredDeviceRepository registeredDeviceRepository, Enum r5) {
        if (r5 == RegistrationStatus.REGISTERED) {
            if (deviceRegistrationRepository.isRegistered()) {
                Log.I("device already registered");
                return;
            }
            Log.I("set device as registered");
            registeredDeviceRepository.save(DeviceId.readDeviceId(this.context), DeviceUtils.readModelName(), SalesCode.get().toString(), new MDMInfo().getCorpId());
    ...
    public void execute() {
        ...
        showDialog("Change Registration Status", RegistrationStatus.values(), RegistrationStatus.REGISTERED, new Consumer() { // from class: com.idm.fotaagent.enabler.ui.admin.main.hidden.command.f
            ...
            public final void accept(Object obj) {
                this.f6408c.lambda$execute$0(deviceRegistrationRepository, registeredDeviceRepository, (Enum) obj);
            }
        });
    }
}
