...
public class Memoryfull extends HiddenCommand {
    ...
    public /* synthetic */ void lambda$showDialogForCreatingSpecificSize$1(EditText editText, int i5, DialogInterface dialogInterface, int i6) {
        ...
        long j3 = GeneralUtils.parseInt(editText.getText().toString(), i5);
        Log.I("remain size : " + j3);
        if (j3 < 0 || j3 > availableBytesAtCache) {
            showToast("wrong input");
            return;
        }
        long availableBytesAtCache2 = this.fileManager.getAvailableBytesAtCache() - (j3 * StorageConstants.MEGA_BYTES);
    ...
    public void execute() {
        showDialog("Select memory type", MemoryType.values(), MemoryType.CACHE_MEMORY, new C2.c(8, this));
    }
}
