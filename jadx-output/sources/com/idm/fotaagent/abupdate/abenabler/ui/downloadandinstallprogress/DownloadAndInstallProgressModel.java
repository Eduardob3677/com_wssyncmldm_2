...
public class DownloadAndInstallProgressModel {
    ...
    private void resetUpdateInfo() {
        ...
        UpdateStatus.InstallationStep installationStep = UpdateStatus.InstallationStep.DOWNLOADING;
        if (iInstallationStep == installationStep.getStatus()) {
            this.repository.setUpdateInfo(new UpdateInfo(Integer.valueOf(installationStep.getStatus()), getUpdateInfo().percent()));
            return;
        }
        UpdateStatus.InstallationStep installationStep2 = UpdateStatus.InstallationStep.VERIFYING;
        if (iInstallationStep >= installationStep2.getStatus()) {
            this.repository.setUpdateInfo(new UpdateInfo(Integer.valueOf(installationStep2.getStatus()), 0));
        }
    }
    ...
    public boolean needsConfirmToPause() {
        ...
        B.f.D("pausing during download should be allowed or no button. step : ", iInstallationStep);
        return false;
    }
