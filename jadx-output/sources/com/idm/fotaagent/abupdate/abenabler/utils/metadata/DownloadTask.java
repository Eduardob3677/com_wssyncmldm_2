...
public class DownloadTask implements Supplier<MetadataDownloader.MetadataDownloadState> {
    ...
    private void append(String str, RangeRequest rangeRequest) throws MetadataDownloader.DownloadSuspendedException, MetadataDownloader.DownloadFailedException {
        while (true) {
            try {
                ...
                return;
            } catch (HttpResponseCodeException | IOException | RuntimeException | URISyntaxException e5) {
    ...
    private RangeRequest.Range range(long j3) {
        InstallParamRepository installParamRepository = new InstallParamRepository(IDMApplication.idmGetContext());
        return new RangeRequest.Range(installParamRepository.getOffset() + j3, (installParamRepository.getMetadataFileSize() + installParamRepository.getOffset()) - 1);
    }

    public void stopIfRunning() {
        Log.I("");
        this.stopRequested = true;
        Optional.of(this.rangeRequest).map(new b(3)).map(new b(4)).ifPresent(new a(2));
    }
