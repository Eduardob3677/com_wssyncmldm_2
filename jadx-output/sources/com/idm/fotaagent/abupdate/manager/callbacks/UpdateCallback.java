...
public class UpdateCallback extends AbstractCallback {
    private int calculatePercent(float f) {
        if (f > 1.0f) {
            f = 1.0f;
        }
        return (int) (f * 100.0f);
    ...
    private void reportForFailure(ErrorCode errorCode) {
        ...
        errorCode.reportForFailureToDLDM();
    }
    ...
    public void reportForSuccess() {
        ...
        if (new ActionInfoDao(taskId).isTriggeredBySideload()) {
            ...
            IDMFumoExecuteHandler iDMFumoExecuteHandler = new IDMFumoExecuteHandler(taskId);
            DLResultCode dLResultCode = DLResultCode.SUCCESS;
            iDMFumoExecuteHandler.callbackIfPossible(35, dLResultCode.getDmResultCode());
            DLReporter.ForSuccess.getInstance().report(taskId, dLResultCode.getMessage());
        }
    }

    public void setUpdateInfo(int i5, float f) {
        Integer numOrElse = UpdateStatus.InstallationStep.ofNullable(i5).orElse(Integer.valueOf(new FotaJobRepository(getContext()).getUpdateInfo().installationStep()));
        numOrElse.getClass();
        UpdateInfo updateInfo = new UpdateInfo(numOrElse, calculatePercent(f));
        Log.I(updateInfo);
    ...
    public void startDownloadingWith(long j3) {
        EnablerManager.getInstance().applyPayload(j3);
    }
