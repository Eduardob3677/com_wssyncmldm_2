...
import com.idm.fotaagent.receiver.b;
...
public abstract class BatteryChecker {
    ...
    public static class ForAB extends BatteryChecker {
        ...
        public int getBatteryLevelToUpdate() {
            ...
            return new FotaJobRepository(this.context).getUpdateInfo().installationStep() >= UpdateStatus.InstallationStep.VERIFYING.getStatus() ? installParamRepository.getBatteryInstallLevel().orElse(20).intValue() : installParamRepository.getBatteryInstallLevel().orElse(20).intValue() + 10;
        }
    ...
    public static class ForNonAB extends BatteryChecker {
        private static final int BATTERY_LEVEL_FOR_GLOBAL = 20;
        private static final int BATTERY_LEVEL_FOR_TMB = 30;

        /* JADX INFO: Access modifiers changed from: private */
        public int getBatteryLevel() {
            return (Operator.get() == Operator.TMB || Operator.get() == Operator.TMK) ? 30 : 20;
        }

        @Override // com.idm.fotaagent.utils.battery.BatteryChecker
        public int getBatteryLevelToUpdate() {
            return new InstallParamRepository(this.context).getBatteryInstallLevel().orElseGet(new b(2, this)).intValue();
        }
    ...
    private boolean isEnoughBattery(int i5) {
        int iIntValue = ((Integer) Optional.ofNullable((BatteryManager) this.context.getSystemService("batterymanager")).map(new d(6)).orElse(0)).intValue();
