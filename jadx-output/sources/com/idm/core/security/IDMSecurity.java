...
class IDMSecurity implements IDMSecurityInterface, Constants {
    ...
    private char[] convertUint64ToA(long j3, char[] cArr, int i5) {
        int i6;
    ...
    private String idmFBAdpOspGenerateDevPwdKey(String str) {
        ...
        for (int i7 = 0; i7 < i5 - 1; i7++) {
            char c2 = cArr[i7];
            byte b3 = IDMSecurityInterface.szDict[i7];
            j3 += c2 * b3;
            j5 += c2 * cArr[(i5 - i7) - 1] * b3;
        }
        return "".concat(String.valueOf(j3)).concat(String.valueOf(j5));
    }
