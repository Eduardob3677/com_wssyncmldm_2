...
public class FotaTelephonyManager {
    ...
    public static int getDataSimSlotId(Context context) {
        if (isDualSim()) {
            return ((Integer) Optional.ofNullable((SubscriptionManager) context.getSystemService(SubscriptionManager.class)).map(new d(7)).map(new d(8)).orElseGet(new b(2))).intValue();
        }
    ...
    public static String getIMEI(Context context) {
        ...
        try {
            return Build.VERSION.SDK_INT >= 34 ? telephonyManager.getPrimaryImei() : telephonyManager.getImei(0);
