...
public class CarrierFeature {
    ...
    private static final String SYNCML_ENABLE_NOTI_DELTA_BINARY_SIZE_BEFORE_DOWNLOAD = "CarrierFeature_SyncML_EnableNotiDeltaBinarySizeBeforeDownload";
    ...
    public interface Delegate {
        ...
        public static class Impl implements Delegate {
            ...
            @Override // com.samsung.android.fotaagent.common.feature.CarrierFeature.Delegate
            public boolean isShowDeltaSizeBeforeDownload(int i5) {
                return CarrierFeature.getCarrierFeature(i5, CarrierFeature.SYNCML_ENABLE_NOTI_DELTA_BINARY_SIZE_BEFORE_DOWNLOAD, false);
            }
        }
        ...
        @FeatureAttribute(description = "Whether to have to notify the size of delta binary (aka., update package).\nUsers can decide whether to download delta binary from its size.\n\n1. true: notify the size\n2. false: need not notify the size.\n\nNote that the size could be notified even in the case of FALSE due to other features.", values = {"true", "false"})
        boolean isShowDeltaSizeBeforeDownload(int i5);
    }
    ...
    public static boolean isShowDeltaSizeBeforeDownload() {
        return isShowDeltaSizeBeforeDownload(FotaTelephonyManager.getDataSimSlotId(CONTEXT));
    }
    ...
    private static boolean isShowDeltaSizeBeforeDownload(int i5) {
        return DELEGATE.isShowDeltaSizeBeforeDownload(i5);
    }
