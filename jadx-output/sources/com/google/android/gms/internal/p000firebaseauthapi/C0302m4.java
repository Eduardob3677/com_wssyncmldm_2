package com.google.android.gms.internal.p000firebaseauthapi;

import c1.AbstractC0213D;
import org.json.JSONException;
import org.json.JSONObject;

/* renamed from: com.google.android.gms.internal.firebase-auth-api.m4, reason: case insensitive filesystem */
/* loaded from: classes.dex */
public final class C0302m4 implements X3 {

    /* renamed from: c, reason: collision with root package name */
    public final /* synthetic */ int f5651c;

    /* renamed from: d, reason: collision with root package name */
    public final String f5652d;

    public C0302m4(String str, int i5) {
        this.f5651c = i5;
        switch (i5) {
            case 1:
                AbstractC0213D.c(str);
                this.f5652d = str;
                break;
            case 2:
                AbstractC0213D.c(str);
                this.f5652d = str;
                break;
            case 3:
                AbstractC0213D.c(str);
                this.f5652d = str;
                break;
            default:
                AbstractC0213D.c(str);
                this.f5652d = str;
                break;
        }
    }

    @Override // com.google.android.gms.internal.p000firebaseauthapi.X3
    public String a() throws JSONException {
        switch (this.f5651c) {
            case 1:
                JSONObject jSONObject = new JSONObject();
                jSONObject.put("idToken", this.f5652d);
                return jSONObject.toString();
            case 2:
                JSONObject jSONObject2 = new JSONObject();
                jSONObject2.put("grantType", "refresh_token");
                jSONObject2.put("refreshToken", this.f5652d);
                return jSONObject2.toString();
            default:
                JSONObject jSONObject3 = new JSONObject();
                jSONObject3.put("idToken", this.f5652d);
                return jSONObject3.toString();
        }
    }
}
